...
const jadibtsFolder = './NovaJadiBot'
const tarPath = './backup.tar.gz'
let option = parseInt(text)

if (![1, 2, 3].includes(option))
return await m.reply(
`*⚠️ ¿QUÉ HAGO? UN BACKUP DE LA SESIÓN O BASE DE DATOS?*. USAR DE LA SIGUIENTE MANERA. EJEMPLO:\n${usedPrefix + command} 1 _(Enviar la session "creds.json")_\n${usedPrefix + command} 2 _(Enviar la base de datos)_\n${usedPrefix + command} 3 _(Envía la carpeta "NovaJadiBot" completa)_`
)
...
if (option === 1) {
const path = conn.user.jid !== global.conn.user.jid ? `./NovaJadiBot/${conn.user.jid.split`@`[0]}/creds.json` : './NovaBotSession/creds.json'
...
if (!fs.existsSync(jadibtsFolder)) return await m.reply('⚠️ La carpeta *NovaJadiBot* no existe.')
await m.reply('_*📂 Preparando la carpeta "NovaJadiBot"...*_')
const output = fs.createWriteStream(tarPath)
const archive = archiver('tar', {gzip: true, gzipOptions: {level: 9}})
output.on('close', async () => {
console.log(`Archivo .tar.gz creado: ${archive.pointer()} bytes`)
await conn.reply(m.sender, `📂 *Carpeta "NovaJadiBot"* (${date})`, fkontak)
await conn.sendMessage(
m.sender,
{document: fs.readFileSync(tarPath), mimetype: 'application/gzip', fileName: 'NovaJadiBot.tar.gz'},
{quoted: m}
)
fs.unlinkSync(tarPath)
})
...
let creds = await fs.readFileSync(`./NovaBotSession/creds.json`)
